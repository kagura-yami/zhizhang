...
const parseLocalDateTime = (dateText: string, timeText: string) => {
...
const CreateBillScreen: React.FC<CreateBillScreenProps> = ({ navigation, route }) => {
  ...
  const initialDateTime = editingBill
    ? new Date(editingBill.time || editingBill.date || editingBill.createdAt)
    : new Date();
  const safeInitialDateTime = Number.isNaN(initialDateTime.getTime()) ? new Date() : initialDateTime;
  const [billDate, setBillDate] = useState(formatLocalDate(safeInitialDateTime));
  const [billTime, setBillTime] = useState(formatLocalTime(safeInitialDateTime));
  ...
  const validateForm = () => {
    ...
    if (!parseLocalDateTime(billDate, billTime)) {
      showError('请输入有效的账单日期和时间');
      return false;
    }
    return true;
  };
  ...
  const useCurrentDateTime = () => {
    const now = new Date();
    setBillDate(formatLocalDate(now));
    setBillTime(formatLocalTime(now));
  };

  const handleDateTimeChange = (event: DateTimePickerEvent, selectedValue?: Date) => {
    const activeMode = pickerMode;
    setPickerMode(null);
    if (event.type === 'dismissed' || !selectedValue || !activeMode) return;
    if (activeMode === 'date') setBillDate(formatLocalDate(selectedValue));
    if (activeMode === 'time') setBillTime(formatLocalTime(selectedValue));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      const occurredAt = parseLocalDateTime(billDate, billTime)!;
      const billData = {
        ...
        <View style={styles.content}>
          ...
          <View style={styles.formGroup}>
            <View style={styles.formLabelRow}>
              <Text style={styles.formLabel}>账单时间</Text>
              <TouchableOpacity style={styles.nowButton} onPress={useCurrentDateTime} accessibilityRole="button">
                <Text style={styles.nowButtonText}>设为现在</Text>
              </TouchableOpacity>
            </View>
            <Text style={styles.formHint}>点击日期或时间，使用手机系统选择器调整</Text>
            <View style={styles.dateTimeRow}>
              <View style={styles.dateField}>
                <Text style={styles.inputCaption}>日期</Text>
                <TouchableOpacity
                  style={styles.dateTimeButton}
                  onPress={() => setPickerMode('date')}
                  accessibilityLabel="账单日期"
                >
                  <CalendarDays size={19} color={styles._colors.primary} />
                  <Text style={styles.dateTimeValue}>{billDate}</Text>
                </TouchableOpacity>
              </View>
              <View style={styles.timeField}>
                <Text style={styles.inputCaption}>时间</Text>
                <TouchableOpacity
                  style={styles.dateTimeButton}
                  onPress={() => setPickerMode('time')}
                  accessibilityLabel="账单时间"
                >
                  <Clock3 size={19} color={styles._colors.primary} />
                  <Text style={styles.dateTimeValue}>{billTime}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
      ...
      <View style={styles.bottomActions}>
        ...
        <TouchableOpacity style={styles.saveButton} onPress={handleSave} disabled={loading} activeOpacity={0.8}>
          {loading
            ...
            : <Text style={styles.saveButtonText}>{isEditing ? '更新账单' : '保存账单'}</Text>}
        </TouchableOpacity>
      </View>

      {pickerMode && (
        <DateTimePicker
          value={parseLocalDateTime(billDate, billTime) || new Date()}
          mode={pickerMode}
          is24Hour
          display="default"
...
const createStyles = (colors: ThemeColors) => ({
  ...StyleSheet.create({
    ...
    formLabelRow: { minHeight: 40, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
    nowButton: { minHeight: 40, justifyContent: 'center', paddingHorizontal: spacing.sm, borderRadius: borderRadius.small, borderWidth: borderWidth.thin, borderColor: colors.primary },
    nowButtonText: { fontSize: 12, fontWeight: '800', color: colors.primary },
    formHint: { marginBottom: spacing.sm, fontSize: 11, fontWeight: '600', color: colors.textSecondary },
    dateTimeRow: { flexDirection: 'row', gap: spacing.sm },
    dateField: { flex: 1.6, minWidth: 0 },
    timeField: { flex: 1, minWidth: 0 },
    inputCaption: { marginBottom: spacing.xs, fontSize: 11, fontWeight: '700', color: colors.textSecondary },
    dateTimeButton: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingHorizontal: spacing.md, borderRadius: borderRadius.input, borderWidth: borderWidth.thin, borderColor: colors.divider, backgroundColor: colors.surface },
    dateTimeValue: { flex: 1, minWidth: 0, color: colors.textPrimary, fontSize: 14, fontWeight: '800', fontFamily: 'Courier' },
