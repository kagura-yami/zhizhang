...
export default function LedgerNotice({ summary, error, loading, refresh, gridHint = true }: {
  ...
  return <View style={{ margin: 16, gap: 8 }}>
    ...
    {summary && <>
      ...
      {!summary.complete && <>
        <Text style={s.error}>还有 {summary.counts.needsReview} 笔待确认，当前金额不完整。{gridHint ? '格内“待”表示该周期有待确认账单。' : ''}</Text>
