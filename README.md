...
## 项目亮点
#### 亮点1：多入口统一编排
手动、语音、拍照、AI对话和支付通知等不同记账入口，最终统一转换为结构化账单，再进入同一套账务处理流程，实现输入方式与账务规则解耦；支付通知采用“来源识别—信息解析—去重—自动分类—入账”的事件驱动链路，便于后续继续扩展新的记账方式。

#### 亮点2：统一账务语义与混合智能算法
项目设计统一Ledger账务语义层，对收入、支出、退款、转账、调账等资金行为统一处理，并由首页、统计、预算和AI分析共同复用；同时采用“规则引擎 + 统一账务语义 + 大模型推理”的混合智能架构，确定性信息优先由规则处理，文字、语音、图片等非结构化内容交由AI理解，在保证账务准确性的同时提升智能化能力。

#### 亮点3：面向Agent的AI任务编排
AI不只是聊天窗口，而是嵌入记账和财务分析流程。财务复盘基于真实账务与预算历史构建分析依据，并采用持久化任务、后台Worker、任务租约、失败重试及数据变化后的报告失效机制，使AI能力可以稳定运行，也为后续从“智能记账工具”扩展为“个人财务Agent”提供基础。
